...
const resolveMissionSource = (
  ...
  // Legacy rows created before mission.source existed: empty open inspections
  // were typically generated by telemetry and need an upload path.
  return status === 'OPEN' && imageCount === 0 ? 'AUTOMATED' : 'MANUAL';
};

export const canUploadMissionImages = (
  mission: Pick<Mission, 'source' | 'status'>
): boolean => mission.source === 'AUTOMATED' && mission.status === 'OPEN';

const EMPTY_MISSION_CLEANUP_GRACE_MS = 5 * 60 * 1000;
...
interface DeleteMissionImageResult {
  detail?: string;
  mission_deleted?: boolean;
}

const shouldCleanupEmptyMission = (createdAt: string): boolean => {
  const createdTime = new Date(createdAt).getTime();
  return Number.isFinite(createdTime) && Date.now() - createdTime > EMPTY_MISSION_CLEANUP_GRACE_MS;
};

async function missionHasImages(missionId: string): Promise<boolean> {
...
export async function deleteMissionIfEmpty(missionId: string): Promise<boolean> {
  if (await missionHasImages(missionId)) {
    return false;
  }

  try {
    const resp = await apiFetch(`/missions/${missionId}`, {
      method: 'DELETE',
    });
    ...
    if (!([404, 405, 409] as number[]).includes(resp.status)) {
      const text = await resp.text();
      throw new Error(text || `Failed to delete empty mission (${resp.status})`);
    }
  } catch (error) {
    console.warn('Backend empty mission cleanup failed; falling back to Supabase delete:', error);
  }
...
async function cleanupEmptyMissions(missionIds: string[]): Promise<void> {
  await Promise.all(
    missionIds.map(async (missionId) => {
      try {
        await deleteMissionIfEmpty(missionId);
      } catch (error) {
        console.warn(`Failed to delete empty mission ${missionId}:`, error);
      }
    })
  );
}
...
export function useMissions() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const missionSummaries = rows.map((row) => {
        const imageCount = imageCounts.get(row.id) || 0;
        ...
        return {
          ...
          status,
          source: resolveMissionSource(row.source, status, imageCount),
      ...
      const emptyMissionIds = missionSummaries
        .filter((mission) =>
          mission.image_count === 0 &&
          !canUploadMissionImages(mission) &&
          shouldCleanupEmptyMission(mission.created_at)
        )
        .map((mission) => mission.id);

      if (emptyMissionIds.length > 0) {
        void cleanupEmptyMissions(emptyMissionIds);
      }

      return missionSummaries.filter(
        (mission) => mission.image_count > 0 || canUploadMissionImages(mission)
      );
    },
  });
}
...
export function useDeleteMissionImage() {
  ...
  return useMutation({
    mutationFn: async ({ imageId, missionId }: { imageId: string; missionId: string }) => {
      ...
      return (await resp.json()) as DeleteMissionImageResult;
    },
    onSuccess: (result, vars) => {
      queryClient.invalidateQueries({ queryKey: ['mission-images', vars.missionId] });
      queryClient.invalidateQueries({ queryKey: ['missions'] });
      toast.success(
        result?.mission_deleted
          ? 'Image deleted and empty inspection removed'
          : 'Image deleted'
      );
    },
    onError: (err) => {
